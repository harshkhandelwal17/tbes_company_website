...
import {
  ...
  MessageSquare, Clock, ArrowRight, X, Copy, Inbox, Loader2
} from 'lucide-react';
...
export default function ContactsPage() {
  ...
  const [updatingStatus, setUpdatingStatus] = useState<string | null>(null); // tracks which contact is being updated
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    if (updatingStatus === id) return; // Prevent double-submission

    const previousContacts = [...contacts];
    const previousSelected = selectedContact ? { ...selectedContact } : null;

    setUpdatingStatus(id);
    // Optimistic Update
    setContacts(prev => prev.map(c => c._id === id ? { ...c, status: newStatus as any } : c));
    if (selectedContact?._id === id) setSelectedContact({ ...selectedContact, status: newStatus as any });
    ...
    try {
      ...
      if (previousSelected) setSelectedContact(previousSelected);
    } finally {
